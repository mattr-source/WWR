...
test('isolation: one storage key, a strict import allow-list, and no path to the API, wallet or Worker', () => {
  ...
  const SANDBOX_UI = ['react', '../../shared/sandbox', '../../shared/sandboxSeason', '../../shared/upgrades', '../../shared/assets', '../../shared/exercises', '../../shared/season1Ops', '../../shared/assetVisuals', '../../shared/terrainAtlas', '../../shared/allianceConvoyVisuals', '../../shared/base', '../../shared/terrain', '../../shared/terrainProps', '../../shared/chat', '../../shared/season', '../../shared/arena', '../../shared/warfront', '../../shared/allianceConvoy', './store', './flag', './beats', './Battlefield', './SectorMap', './worldGround', './baseRoles', './BasePanels', './Comms', './seasonHub', './SeasonHub', './worldSkins', '../../shared/skins', './original/strings', './original/GameClock', './original/SandboxBaseBoard', './original/SandboxGuide', './original/SandboxPanels', './original/WorldView', './RobotFigure', './RobotBay', './InstallCeremony', './RefitArt', './RefitCeremony', './refitArt.css', './ui', './sandbox.css', './robotFigure.css'];
  // The ported live layouts (src/sandbox/original/) are one folder down: same rules, their own relative paths.
  const SANDBOX_ORIGINAL = ['react', '../../../shared/base', '../../../shared/sandbox', '../../../shared/sandboxSeason', '../../../shared/gametime', '../../../shared/exercises', '../../../shared/season', '../../../shared/assetVisuals', '../../../shared/assets', '../../../shared/drones', '../../i18n/en/base', '../../i18n/en/chat', '../../i18n/en/core', '../../i18n/en/map', './strings', './GameClock', '../SectorMap', '../worldGround', '../ui', '../RobotFigure'];
  const files = ['shared/sandbox.ts', 'shared/sandboxSeason.ts', ...(readdirSync(join(root, 'src/sandbox'), {recursive: true}) as string[]).filter((f) => /\.tsx?$/.test(f)).map((f) => `src/sandbox/${f}`)];
  for (const file of files) {
    const src = readFileSync(join(root, file), 'utf8');
    const imports = [...src.matchAll(/(?:from|import)\s+'([^']+)'/g)].map((m) => m[1]);
    for (const spec of imports) assert.ok((ALLOWED[file] ?? (file.startsWith('src/sandbox/original/') ? SANDBOX_ORIGINAL : SANDBOX_UI)).includes(spec), `${file} imports ${spec}`);
  }
